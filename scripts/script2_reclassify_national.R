...
agg_change <- data.frame(cbind(1:8,c("F","NF","Def_O","Def_C","Def_R","Deg","Enh","Mgt")))
names(agg_change) <- c("final","change_class")

df2 <- merge(df1,agg_change,by.x="final",by.y="final")
df2$area <- df2$pix_count * 30 * 30
write.csv(df2,"all_transitions.csv",row.names = F)
...
#################### COMPARE WITH VECTOR VERSION
shp_change <- readOGR("vector/Change_FC14_16_31_august.shp","Change_FC14_16_31_august")
shp_change@data$area <- gArea(shp_change,byid = T)
dbf <- shp_change@data

write.dbf(dbf,"vector/Change_FC14_16_31_august.dbf")

#################### STATS
stats <- cbind(tapply(dbf$area,dbf$class_t1,sum),tapply(dbf$area,dbf$class_t2,sum))
tapply(dbf$area,dbf$final,sum)
